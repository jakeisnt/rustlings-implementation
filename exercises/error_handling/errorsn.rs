...
fn read_and_validate(b: &mut dyn io::BufRead) -> Result<PositiveNonzeroInteger, Box<dyn error::Error>> {
    // By allowing the type to accept all errors, we get away with this.
    // It would be better practice to define a custom Box enum that can accept all of the errors
    // we care about. This post looks like it provides a good overview: https://stevedonovan.github.io/rust-gentle-intro/6-error-handling.html
